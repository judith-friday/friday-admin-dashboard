        ...
        <div className="flex items-center justify-between">
          ...
          {stats && (
            <div className="flex space-x-5 items-center flex-wrap gap-y-1">
              ...
              <button onClick={() => { setShowTeachingsPanel(!showTeachingsPanel); if (!showTeachingsPanel) fetchTeachings() }} className="ml-1 px-1.5 py-0.5 rounded text-xs" style={{background: 'rgba(168,85,247,0.1)', color: '#c084fc'}} title="Teachings">\uD83E\uDDE0</button><button data-testid="btn-help" onClick={() => setShowHelp(true)} className="ml-1 w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold" style={{background: 'rgba(99,149,255,0.15)', color: '#6395ff'}}>?</button>
            </div>
          )}
        </div>
      </div>

...
function DraftContent({
  draft, editingDraft, setEditingDraft, isEditingRef,
  ...
  draftStateBadge, propertyName, maxHeight,
}: {
  draft: Draft
  maxHeight: string
} & Omit<DraftPanelProps, 'drafts' | 'revisionPending'>) {
  return (
    <>
          ...
          <div className="mt-3">
            ...
            {showTeachPrompt === draft.id && (
              <div className="mt-2 p-2 rounded" style={{background: 'rgba(168,85,247,0.06)', border: '1px solid rgba(168,85,247,0.15)'}}>
                ...
                <div className="flex space-x-2">
                  <button onClick={() => handleRevision(draft.id, 'teach', 'property', propertyName || undefined)}
...
export default function DraftPanel({
  ...
  const sharedProps = {
    editingDraft, setEditingDraft, isEditingRef,
    editBody, setEditBody, revisionText, setRevisionText, revisingDraft,
    rejectingDraft, setRejectingDraft, rejectReason, setRejectReason,
    showTeachPrompt, setShowTeachPrompt,
    requestApproval, handleDraftAction, handleRevision, handleRejectWithReason,
    ...
    <>
      {readyDrafts.map(draft => (
        <React.Fragment key={draft.id}>
          {/* Mobile: collapsible draft bar */}
          <div className="md:hidden">
            <button onClick={() => setDraftExpanded(!draftExpanded)}
              className="w-full flex items-center justify-between px-4 py-2 text-sm flex-shrink-0"
              style={{background: 'rgba(99,149,255,0.06)', borderTop: '1px solid rgba(255,255,255,0.06)'}}>
              <span style={{color: '#94a3b8'}}>
                {'\uD83D\uDCDD'} AI Draft{draft.confidence != null ? ` (${Math.round(Number(draft.confidence))}%)` : ''}
              </span>
              <span style={{color: '#64748b'}}>{draftExpanded ? '\u25BC' : '\u25B2'}</span>
            </button>
            {draftExpanded && (
              <div className="rounded-lg p-3 mx-3 mb-1 flex-shrink-0" style={{background: 'rgba(99,149,255,0.06)', border: '1px solid rgba(99,149,255,0.15)', borderTop: '1px solid rgba(255,255,255,0.06)'}}>
                <DraftContent draft={draft} maxHeight="max-h-[35vh]" {...sharedProps} />
              </div>
            )}
          </div>
          ...
          <div className="hidden md:block">
            <div className="rounded-lg p-3 mx-3 mb-1 flex-shrink-0" style={{background: 'rgba(99,149,255,0.06)', border: '1px solid rgba(99,149,255,0.15)', borderTop: '1px solid rgba(255,255,255,0.06)'}}>
              <DraftContent draft={draft} maxHeight="max-h-[20vh]" {...sharedProps} />
            </div>
          </div>
        </React.Fragment>
      ))}
